...
- **Config file** — `~/.config/synkban/synkban.toml` (`backend/src/config.rs`; dir overridable via `SYNKBAN_CONFIG_DIR`, used by the e2e harness). Holds `data_dir`, `startup_view`, `last_board_id`. Owned by the **backend** in all modes; read/written via `GET`/`PUT /api/settings`. Settings must live server-side: the desktop shell serves the UI from a random port each launch and localStorage is per-origin (scheme+host+port), so client-side persistence never survives a desktop restart.
...
- **Config** in `config.rs` — `~/.config/synkban/synkban.toml` (dir overridable via `SYNKBAN_CONFIG_DIR`): `data_dir`, `startup_view`, `last_board_id`. `load()`/`update(mutate)` with a process-wide `Mutex` serializing read-modify-write. This is the only file I/O outside `store/`, and it's outside the data dir by design (a broken custom `data_dir` must not strand the setting that fixes it).
...
```
GET    /api/settings                          → SettingsResponse (startup_view, last_board_id [self-healing: absent when the board no longer exists/is archived], data_dir, configured_data_dir, default_data_dir)
PUT    /api/settings  {startup_view?, last_board_id??, data_dir??}  → SettingsResponse — writes ~/.config/synkban/synkban.toml; double-Option fields: null clears/reverts, absent leaves unchanged. data_dir change needs a restart to take effect.
...
- **Settings dialog** — gear button in `.app-header-actions` (**board overview only**, hidden on board pages) opens `SettingsDialog` (`.settings-overlay`/`.settings-dialog`), rendered by `App.tsx` as a *sibling* of the header (the header is the Electron drag region and must not contain the overlay). All edits are **buffered locally**; nothing persists until Save (single `PUT /api/settings`) — Cancel, Escape, X and overlay click all discard. Sections: **On startup, open** (card-style radios: board overview vs. last used board) and **Data folder** (Electron: path box + Browse…/Use default via the `synkbanDesktop` bridge, staged until Save; saving a folder change shows "Save & Restart" and relaunches the app; web mode: read-only hint naming `--data-dir`/`DATA_DIR`/`synkban.toml` + current path). Escape via `dialogKeys`, focus containment via `focusTrap`. Covered by `e2e/settings.spec.ts` (runs against a **dedicated server instance** on :8092 — settings are global server state and would race the parallel specs' `goto("/")`).
