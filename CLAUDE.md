...
- **`store/`** is the only place that touches the filesystem for data. Handlers call `store::*` functions — `mod.rs` re-exports a flat public API so handlers don't care about the internal layout. Submodules:
  ...
  - `store/attachments.rs` — attachment binary I/O + thumbnail generation
...
- **Module layout:**
  ...
  - `components/` — leaf and section components. `Card`, `List`, `CardDetail`, `AddForm`, `ShortcutHelp`; board-level sections (`LabelDrawer`, `ArchiveCardsModal`, `FilterBar`, `BoardColorPicker`, `ConfirmDialog`); card-detail sections (`CardLabelSection`, `DueDateSection`, `AttachmentsSection`, `ImagePreviewOverlay`, `UnsavedDialog`)
  - Shared utilities at `src/` root: `boardInput.ts` (`isInInput` guard), `mdInput.ts` (`**bold**`/`*italic*` Ctrl+B/I shortcut for plain inputs), `proseEditor.ts` (ProseMirror schema + `createCardEditor`), `positions.ts` (fractional indexing math), `filter.ts` (card filter predicate), `focusTrap.ts` (dialog focus containment, see pitfall below)
  - `LabelContext.tsx` — provides label-drawer open state and board-title rename state across `App`/`Board`
...
- Card `checklist: Vec<ChecklistItem>` — single flat checklist per card, stored in card JSON (no separate file). Each item has `id` (UUID, server-generated), `text`, `done: bool`. Vec order is display order. Uses `#[serde(default)]` so legacy cards deserialize with an empty checklist. Managed via dedicated sub-resource endpoints (`/api/cards/:id/checklist[/:item_id]`); changes save immediately from the UI (not part of the card Save flow). `PUT /api/cards/:cid/checklist` with `{ done: bool }` checks/unchecks all items at once.
...
POST   /api/cards/:cid/checklist  {text}       → ChecklistItem (201)
PUT    /api/cards/:cid/checklist  {done}       → Card — check/uncheck all items
PUT    /api/cards/:cid/checklist/:item_id {text?,done?,pos?} → ChecklistItem — pos = target index (clamped), reorders item
DELETE /api/cards/:cid/checklist/:item_id      → 204
...
- **Dialog focus containment** — `focusTrap.ts` exports `focusTrap(root)`; every overlay dialog attaches it via `ref={(el) => onCleanup(focusTrap(el))}` (CardDetail `.modal-overlay`, `UnsavedDialog`, `ConfirmDialog`, `ShortcutHelp`, `ArchiveCardsModal`, `ImagePreviewOverlay`, Home archived-boards overlay). It guarantees keyboard focus never escapes an open dialog: (1) if the focused element disappears (inline confirmations, nested dialogs closing) or focus lands on `<body>`, focus is restored to the most recently focused element inside the root that still exists (tracked via `focusin` history; a `MutationObserver` catches removals because browsers don't reliably fire `focusout` when the focused element is removed); (2) `Tab`/`Shift+Tab` wrap at the dialog edges instead of escaping to the page behind. Nested traps compose — the inner dialog handles Tab first and outer handlers skip `defaultPrevented` events. As a defense-in-depth, Board.tsx's global Escape (which only fires when focus is outside the modal) dispatches a `request-card-close` `CustomEvent` instead of closing directly; CardDetail listens and routes it through the unsaved guard. Covered by `e2e/focus.spec.ts`.
