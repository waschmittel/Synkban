...
- `electron/main.js` — Electron main process. Generates a UUID token, spawns the Rust binary with `DESKTOP_TOKEN=<token>` and `DATA_DIR=<dataDir>` env vars, reads `DESKTOP_PORT=<port>` from stdout, then opens `BrowserWindow` at `http://127.0.0.1:<port>/?token=<token>`.
- `electron/package.json` — `electron-builder` config. Bundles Rust binary as `extraResources` alongside the Electron app. Icons from `backend/icons/` — macOS uses `icon.icns`, Windows `icon.png` (electron-builder converts it to a 7-frame `.ico` inside the exe), Linux the **`backend/icons/png/` set** (`<size>x<size>.png`, 16–1024, generated with `sips -z $s $s icon.png`). Linux must be the whole directory: a single PNG makes electron-builder emit a one-entry icon "set" at that PNG's own size, so a 1024px source installed only to `hicolor/1024x1024/`, a directory no hicolor theme declares — `Icon=synkban` then resolves to nothing and the app has no menu/taskbar icon. Also needed for the dock icon: top-level `desktopName` in `package.json`, since Electron derives the window's app_id from it and electron-builder writes the matching `StartupWMClass` (without it the entry said `Synkban` while the window reports `synkban`, so the running window never linked to its launcher entry). `main.js` additionally sets a `BrowserWindow` `icon` on Linux (un-integrated AppImages have no installed `.desktop` to fall back on) and calls `app.setAppUserModelId('com.synkban.app')` for Windows taskbar grouping. Guarded by `e2e/electron.spec.ts`.
...
- **Config file** — `~/.config/synkban/synkban.toml` (`backend/src/config.rs`; dir overridable via `SYNKBAN_CONFIG_DIR`, used by the e2e harness). Holds `data_dir`, `startup_view`, `last_board_id`, `theme`. Owned by the **backend** in all modes; read/written via `GET`/`PUT /api/settings`. Settings must live server-side: the desktop shell serves the UI from a random port each launch and localStorage is per-origin (scheme+host+port), so client-side persistence never survives a desktop restart.
- **Data directory resolution** (both modes, `main.rs`): `--data-dir` CLI flag > `DATA_DIR` env > `data_dir` in `synkban.toml` > default `~/.config/synkban/data`. Electron does **not** pass `DATA_DIR`; the binary resolves it itself. Changing the folder in the Settings dialog PUTs `data_dir` to the config, then relaunches the app (desktop) so the respawned backend picks it up. Existing data is not moved.
- **IPC is shell-only** — all board/card/settings communication is HTTP via the embedded Actix server. The only exceptions are the two operations the web UI can't do: `electron/preload.js` exposes `window.synkbanDesktop` (`pickDataDir` → native directory picker, `relaunch`) via `contextBridge`, handled by `synkban:*` `ipcMain` handlers in `main.js`. The frontend feature-detects the bridge (`settings.ts::desktopBridge()`) and shows a read-only hint instead of the picker in web mode.
- **Seamless title bar** — Electron creates the window without a native frame so the app's own coloured `.app-header` extends to the top edge and doubles as the title bar. macOS uses `titleBarStyle: 'hiddenInset'` with `trafficLightPosition: { x: 14, y: <centred> }`, Windows/Linux `titleBarStyle: 'hidden'`; **`titleBarOverlay` is set on every platform** (colours — transparent fill, white symbols — only apply on Windows/Linux; `height` matches `.app-header`'s 52px, keep the `HEADER_HEIGHT` constant in `main.js` in sync). Enabling it everywhere is what publishes the **`env(titlebar-area-*)`** CSS variables, including on macOS.
  - **Keeping clear of the window controls is pure CSS — no JS measuring.** The controls sit on different sides per OS (macOS left, Windows right, Linux wherever the desktop environment's `gtk-decoration-layout` puts them, so it can be either), so `app.css` never hardcodes a side: `padding-left: max(16px, env(titlebar-area-x, 0px))` and `padding-right: max(16px, 100vw - env(titlebar-area-x, 0px) - env(titlebar-area-width, 100vw))`. The overlay reports the strip actually *left over* for the app, so a rect starting past 0 means controls on the left and a rect narrower than the window means controls on the right — one formula covers all three platforms (and RTL). The browser keeps `env()` live, so this tracks resize/maximise for free.
  - `max(16px, …)` keeps the normal gutter on whichever side has no controls, and covers the cases where the vars are **unset**: fullscreen (controls hidden) and plain web mode (no controls at all). This is why fullscreen no longer leaves a dead inset.
  - Frontend bootstrap (`index.tsx`) only adds `html.electron` (UA sniff for `Electron/`); CSS uses it to scope `-webkit-app-region: drag` on `.app-header` (with `no-drag` on its buttons/inputs/links) and the padding above. There is no `html.electron--mac` class and no platform branch in the frontend — the env vars make it unnecessary.
  - **Traffic lights are centred vertically in the header**, not offset by a magic number: `trafficLightPosition` takes a *top* offset, so `main.js` derives it as `(HEADER_HEIGHT - TRAFFIC_LIGHT_HEIGHT) / 2` = `(52 - 14) / 2` = 19. `TRAFFIC_LIGHT_HEIGHT` is 14px — measured off a screenshot of the real window, **not** the 12px the buttons are usually quoted as. (Once `titleBarOverlay.height` matches the header, y=19 is also what macOS picks by itself; the previous hardcoded `y: 16` sat 3px high.) The x stays 14 for horizontal breathing room, which is why the reserved inset is a few px wider than the buttons themselves.
  - Verified on macOS by driving the real app (Playwright `_electron`) and measuring screenshots: inset 88px at 1200px wide, 83px at 760/900px, 16px in fullscreen; button centre 26.0 in a 52px header. Note 88 ≠ the old hardcoded 84 because `env()` accounts for `trafficLightPosition`.
...
- **Config** in `config.rs` — `~/.config/synkban/synkban.toml` (dir overridable via `SYNKBAN_CONFIG_DIR`): `data_dir`, `startup_view`, `last_board_id`, `theme`. `load()`/`update(mutate)` with a process-wide `Mutex` serializing read-modify-write. This is the only file I/O outside `store/`, and it's outside the data dir by design (a broken custom `data_dir` must not strand the setting that fixes it).
...
```
GET    /api/settings                          → SettingsResponse (startup_view, theme, last_board_id [self-healing: absent when the board no longer exists/is archived], data_dir, configured_data_dir, default_data_dir)
PUT    /api/settings  {startup_view?, theme?, last_board_id??, data_dir??}  → SettingsResponse — writes ~/.config/synkban/synkban.toml; theme ∈ {system,light,dark} (validated, 400 otherwise); double-Option fields (last_board_id/data_dir): null clears/reverts, absent leaves unchanged. data_dir change needs a restart to take effect.
...
- **`isInInput` guard** — `boardInput.ts` exports `isInInput(target)` which returns true if the event target is an INPUT, TEXTAREA, contentEditable element, or inside `.modal-overlay`, `.label-drawer`, `.shortcut-help-overlay`, `.archive-overlay`, `.settings-overlay`, or `.filter-bar`. All global keydown handlers check this before acting, so shortcuts don't fire while typing or when a dialog is open. Covered by `boardInput.test.ts`.
- **Errored resources in effects** — reading an errored SolidJS resource (`board()`, `boards()`) **throws**, and an uncaught throw inside a `createEffect` aborts the whole effect-queue run — *later* effects watching the same resource silently never run for that tick. Every effect that reads a resource must either check `resource.error` first or wrap the read in try/catch (see `focusRestoration.ts`, `Home.tsx` pending-focus effect). This bit as a real bug: the last-board-id clearing effect in `Board.tsx` never ran on 404 because `focusRestoration`'s effect threw first.
- **Settings dialog** — gear button in `.app-header-actions` (**board overview only**, hidden on board pages) opens `SettingsDialog` (`.settings-overlay`/`.settings-dialog`), rendered by `App.tsx` as a *sibling* of the header (the header is the Electron drag region and must not contain the overlay). All edits are **buffered locally**; nothing persists until Save (single `PUT /api/settings`) — Cancel, Escape, X and overlay click all discard. Sections: **Appearance** (card-style radios: Use system setting / Light / Dark — see Dark mode), **On startup, open** (board overview vs. last used board) and **Data folder** (Electron: path box + Browse…/Use default via the `synkbanDesktop` bridge, staged until Save; saving a folder change shows "Save & Restart" and relaunches the app; web mode: read-only hint naming `--data-dir`/`DATA_DIR`/`synkban.toml` + current path). The Appearance radios are the one exception to pure buffering: they **preview live** (each selection calls `theme.ts::applyTheme` immediately, like `BoardColorPicker`), and every discard path routes through a local `close()` that calls `revertThemePreview()` to roll `<html>` back to the persisted value; Save commits the previewed theme and skips the revert. Escape via `dialogKeys`, focus containment via `focusTrap`. Covered by `e2e/settings.spec.ts` (runs against a **dedicated server instance** on :8092 — settings are global server state and would race the parallel specs' `goto("/")`).
- **Dark mode** — persisted server-side as `theme` in `synkban.toml` (`system` [default] | `light` | `dark`); `system` follows the OS via `prefers-color-scheme`. The whole dark palette hangs off a single **`html.theme-dark`** class so the stylesheet stays DRY (one dark block at the bottom of `app.css`, no duplicated media queries). `frontend/src/theme.ts` owns application: `applyTheme(theme)` toggles the class (resolving `system` via `matchMedia` and staying subscribed so an OS appearance change flips the app live), `isDark(theme)` reports the current resolution. Flash-free: an inline `<script>` in `index.html` sets `theme-dark` from the OS **before first paint**; `index.tsx::bootstrap` then applies the persisted preference (fetched once, non-render-blocking, reusing the `/` settings fetch) — this corrects the OS default only if the user forced light/dark. Most surfaces flip automatically because `app.css` routes colours through the semantic tokens in `:root` (which the `html.theme-dark` block reassigns); the dark block only re-specifies **bespoke** light values (gradients, translucent `rgba` overlays that vanish on dark, pastel info boxes, the ProseMirror sticky-note tint, code/kbd chips) plus `color-scheme: dark` (which darkens native controls/scrollbars/date+colour pickers and gives bare inputs a dark field for free). Primary blue `#0079bf` is intentionally left un-tokenised (it reads fine as a fill with white text and as borders/rings on dark); a handful of accent-text usages are brightened to `#6cb0ec` in the dark block. **Board background uses a *darker* version of the board colour** in dark mode: `.board-page` is `color-mix(var(--board-color) 42%, #0d0f14)` (mixed toward black) vs. the light-mode `color-mix(var(--board-color) 50%, white)`. Theme persistence, live preview, and discard are covered by `e2e/settings.spec.ts`.
- **Startup view preference** — persisted server-side in `synkban.toml` (`startup_view`, `last_board_id`), NOT in localStorage (see Config file for why). `Board.tsx` records the id on every successful board load via `settings.ts::recordLastBoard` (deduped per page load so 15s-polling refetches don't rewrite the config). Stale ids self-heal server-side: `GET /api/settings` reports `last_board_id` only if it resolves to an active board. `index.tsx::bootstrap` fetches settings (only when the path is `/`) and rewrites `/` to `/board/<id>` via `history.replaceState` **before the router initializes**, so only a fresh page load bounces — in-app navigation back to the overview never redirects, deep links win, and the query string (desktop `?token=`) is preserved. Covered by `e2e/settings.spec.ts`.
...
- **Board color** — `Board` and `BoardDetail` have an optional `color` field (`Option<String>` in Rust, `color?: string` in TS). Set via `PUT /api/boards/:id` with `{ color }`. `Board.tsx` syncs the value to `--board-color` CSS custom property on `:root` via `createEffect`. CSS uses `linear-gradient(rgba(0,0,0,0.2), ...), var(--board-color)` for the header (dark overlay for readability) and `color-mix(in srgb, var(--board-color) 50%, white)` for the board page background (muted/pastel; in dark mode this mixes toward black instead — see Dark mode). `BoardColorPicker` is a native `<input type="color">` for free color selection (no presets) plus a "Reset to default" button. Its `onInput` calls `onPreview` (Board.tsx sets `--board-color` live, no API call) and `onChange` calls `onSelect` (commits via `PUT /api/boards/:id`); Reset commits `{ color: null }`. `onCleanup` resets `--board-color` to `#0079bf`. Home page board cards show their color as card background via inline style.
