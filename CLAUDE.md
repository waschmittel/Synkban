...
- **Board rename** — The board title is displayed in the app header (replacing "Synkban" when on a board page). Clicking the header board title dispatches a `"start-board-rename"` `CustomEvent`; `Board.tsx` listens for it and activates an inline `input.board-title-input` in the board-title-bar. Blur or Enter commits (calls `api.updateBoard`); Escape cancels. State: `showRename` + `renameValue` signals in `Board.tsx`. A back chevron (`<A href="/" class="app-logo-home">`) appears before the board title in the header for navigation to the home page. The `h2.board-title-text` no longer exists — the title bar only shows the rename input (when active) and action buttons (filter, archive, color).
- **Archive (soft delete)** — Cards have `archived: bool` field. The archive button (`.card-archive`) and `Delete`/`Backspace` key both show a confirmation dialog (`.archive-overlay` + `.unsaved-dialog`) before calling `api.archiveCard`. Archived cards are filtered out by `get_board` on the backend. "Archive" button in the board title bar (or `a` key) opens an `.archive-modal-overlay` listing all archived cards with Restore and Delete buttons. Archive items have `tabindex="0"` and support `↑↓` arrow key navigation. Delete shows inline "Delete permanently?" confirmation (Yes/No). Restore calls `api.restoreCard` → `PUT /api/cards/:id` with `{ archived: false, list_id? }`. Orphaned cards (from deleted lists) need `list_id` — frontend sends first available list. Delete calls `api.deleteCard` → `DELETE /api/cards/:id` (permanent, also removes attachment files). API rejects deletion of non-archived cards (400). Backend uses `CardLocation` enum (`InList` vs `Orphaned`) to locate cards in either list dirs or `archived_cards/`.
